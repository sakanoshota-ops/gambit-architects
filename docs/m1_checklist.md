...
### 1.1 ガンビット評価器（最重要）
- [ ] DSL v0.3 の **条件21種すべて**が予測可能に評価される（テスト通過）
- [ ] DSL v0.3 の **対象6種すべて**が正しく解決される（テスト通過）
- [ ] **行動15種すべて**が `Action` 型として定義され、識別される（実効果は §1.4 のみで OK）
- [ ] 評価器が**純粋関数**として実装されている（同じ入力で同じ出力）
- [ ] ルールはトップ優先、フォールスルー（対象不在・MP不足）が仕様通り動く

### 1.2 ユニットテスト
- [ ] 評価器のユニットテストが書かれている（DSL仕様書 §11 の観点を最低カバー）
- [ ] テストは**1コマンド**で実行できる（`pnpm test`）
- [ ] テストが**全 PASS**

### 1.3 最小戦闘ループ
- [ ] 4 vs N の戦闘を**ターン制で進行**できる（ATB は M2 でよい）
- [ ] 各ユニットが自分のガンビットセットに従って毎ターン行動を選択
- [ ] HP 0 で戦闘不能、敵全滅で勝利、味方全滅で敗北を判定
- [ ] 戦闘の各ターンが**コンソールにテキスト出力**される（見やすさは問わない）
...
| `DEFEND` | ✅ 当ターン被ダメ -50% |
| `WAIT` | ✅ 何もしない |
| `USE_ITEM(POTION)` | ✅ HP回復 |
| その他11種 | データ定義のみ、実行されると `[NotImplemented: SKILL]` のようなログを出して空振り |

### 1.5 最小データ（剣士1ジョブ・敵3〜5種）
- [ ] ジョブ「剣士」のテンプレ（HP/MP/物理/防御）が定義されている
- [ ] 味方ユニット 4 体を剣士テンプレでインスタンス化できる
- [ ] 敵テンプレが **3〜5種**（弱い／普通／強い、ボスなし）定義されている
- [ ] 戦闘開始関数が「味方4体 + 敵 N 体」を受け取って戦闘を実行できる

### 1.6 サンプルガンビット
- [ ] 標準プリセット 4種（DSL §8）が**読み込み可能**な形でデータとして存在する
- [ ] 4体の味方それぞれに別のプリセットを当てて、戦闘をしたとき**手書きと違わない動き**をする
...
- [ ] README に「ローカル起動手順」が3行以内で書かれている
- [ ] 1コマンドで戦闘デモが動く（`pnpm run demo`：コンソールに戦闘ログ出力）
